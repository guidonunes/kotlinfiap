...
import com.example.kotlinfiap.repository.getCategoryById
import com.example.kotlinfiap.repository.getReviewsByCategory
...
fun ReviewCategoryScreen(categoryId: Int?, navController: NavHostController?) {
    ...
    when (reviewsByCategory.size) {
        0 -> {
            ...
            categoryName = reviewsByCategory[0].category.name
        }
    }
            ...
            Column(
                ...
                LazyColumn(
                    ...
                    if (reviewsByCategory.isNotEmpty()) {
                        items(reviewsByCategory) { review ->
                            CategoryReview(review)
                        }
                    } else {
                        item {
                            Text(
                                text = "There are no reviews in this category.",
...
@Preview
@Composable
private fun CategoryReviewPreview() {
    KotlinfiapTheme() {
        CategoryReview(getAllReviews()[0])
    }
}
